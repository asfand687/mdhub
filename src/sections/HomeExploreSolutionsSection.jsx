  ...
  return (
    <section className="flex items-center pb-20">
      <article className="flex-1 relative">
        <img className="relative z-10 max-w-[650px]" src={MedicalBenefitsInfograph} alt="medical benefits inforgraph"/>
        <img className="absolute left-0 -bottom-6 2xl:-bottom-10" src={MedicalBenefitsBg} alt="Medical Benefits Background"/>
        <FiArrowUpRight className="text-2xl absolute bottom-0 left-20"/>
      </article>
      <article className="flex-1 relative font-main bg-gray-100">
        <div>
          <img className="w-full" src={NeverWaitForDoctorAgainImage} alt="never wait for doctor again"/>
          <h2 className="text-4xl absolute z-10 top-6 left-8">Never wait for a <br/> doctor again.</h2>
        </div>
        <div className="p-10 font-body text-2xl">
          <p className=" text-gray-700 w-full tracking-wide max-w-[440px] pb-24">Spend more time with the people you love and less time in waiting rooms with MDHub.</p>
